    ...
    const cargarDatos = async () => {
      ...
      const cierresData = snapshot.docs
        ...
        .filter((c) => {
          ...
          return isValid(fecha) && getMonth(fecha) === mesSeleccionado && getYear(fecha) === anioSeleccionado;
        });
      ...
      // 🔴 AGRUPACIÓN Y ACUMULACIÓN POR REPARTIDOR
      ...
      // 👇 Ahora porRepartidor tiene UN registro por email, acumulado
      setPorRepartidor(Object.values(acumuladoPorRepartidor));

      // 🔽 Resumen por vendedor (entregados + no entregados)
      // 📌 1) Cargar catálogo de productos (id -> {precio, nombre}) y también por nombre normalizado
...
const norm = (s) =>
  ...
  const n = norm(nombre);
  // palabras clave para excluir líneas que no son productos
  const keywords = [
    "envio", "envío", "delivery", "flete",
    "recargo", "transferencia 10", "transferencia10", "interes", "interés",
    "tarjeta", "posnet", "comision tarjeta", "comisión tarjeta"
  ];
...
// 📌 2) Resumen por vendedor (solo valor de productos)
...
cierresData.forEach((cierre) => {
  ...
  (cierre.pedidosEntregados || []).forEach((pedido) => {
    ...
    // Subtotal SOLO de productos (sin envío, sin extras, sin 10%)
...
if (Array.isArray(pedido.productos)) {
  for (const item of pedido.productos) {
    ...
    // 🔴 Si es envío/recargo/etc, lo saltamos
    if (debeExcluirItem(nombreItem)) continue;

    // Preferir precio guardado en el pedido; si falta, buscar en catálogo por id o nombre
  ...
  // No entregados (solo cuentan para la métrica, no suman ventas)
...
// 📌 3) Comisión y ticket promedio (sobre montoProductos)
...
<td>${(v.comision || 0).toLocaleString("es-AR", { minimumFractionDigits: 2 })}</td>
<td>${(v.promedioTicket || 0).toLocaleString("es-AR", { minimumFractionDigits: 2 })}</td>
                </tr>
              ))}
            </tbody>
          </table>
