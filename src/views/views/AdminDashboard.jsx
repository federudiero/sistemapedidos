    ...
    <div className="min-h-screen bg-base-100 text-base-content">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
            <label className="form-control w-full sm:w-44">
              <span className="label-text mb-1 text-xs opacity-70">Fecha</span>
              <input
                type="date"
        ...
        <section className="mt-6 grid gap-5 xl:grid-cols-3">
          <div className="card border border-base-300 bg-base-100 shadow-sm xl:col-span-1">
            <div className="card-body p-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                <h2 className="card-title text-base">Top vendedores</h2>
              ...
              {stats.topVendedores.length ? (
                  ...
                  <table className="table table-sm">
                    ...
                    <tbody>
                      {stats.topVendedores.map((row) => (
                        <tr key={row.key}>
                          <td className="max-w-44 truncate" title={row.key}>{row.key}</td>
                          <td className="text-right">{row.cantidad}</td>
                          <td className="text-right font-semibold">{formatMoney(row.monto)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="rounded-xl bg-base-200 p-4 text-sm opacity-70">Sin pedidos para esta fecha.</div>
              )}
            </div>
          </div>
          ...
          <div className="card border border-base-300 bg-base-100 shadow-sm xl:col-span-1">
            <div className="card-body p-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                <h2 className="card-title text-base">Repartidores</h2>
              ...
              {stats.topRepartidores.length ? (
                    ...
                    <tbody>
                      {stats.topRepartidores.map((row) => (
                        <tr key={row.key}>
                          <td className="max-w-44 truncate" title={row.key}>{row.key}</td>
                          <td className="text-right">{row.cantidad}</td>
                          <td className="text-right font-semibold">{row.entregados}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="rounded-xl bg-base-200 p-4 text-sm opacity-70">No hay repartidores asignados.</div>
              )}
            </div>
          </div>
          ...
          <div className="card border border-base-300 bg-base-100 shadow-sm xl:col-span-1">
            <div className="card-body p-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                <h2 className="card-title text-base">Stock crítico</h2>
              ...
              {stats.stockCritico.length ? (
                <div className="space-y-2">
                  {stats.stockCritico.map((p) => (
                    <div key={p.id} className="flex items-center justify-between gap-3 rounded-xl border border-base-300 bg-base-200/40 p-3">
                      <div className="min-w-0">
                        <div className="truncate font-semibold" title={p?.nombre}>{p?.nombre || "Sin nombre"}</div>
                ...
                <div className="rounded-xl bg-success/10 p-4 text-sm text-success">No hay productos bajo mínimo.</div>
              )}
            </div>
          </div>
        </section>
