...
export default function LoginVendedor() {
  ...
  const esVendedor = async (email) => {
    ...
    const vendedores = toArray(data.vendedores).map((e) => String(e).toLowerCase());
  ...
  const loginEmailPass = async () => {
    ...
    try {
      ...
      if (!ok) { await signOut(auth); return setError("❌ Este correo no está autorizado como vendedor en esta provincia."); }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.error(err); setError("❌ Error al iniciar sesión con Google.");
    } finally { setLoadingLogin(false); }
  };
